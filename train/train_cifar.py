...
if __name__ == '__main__':
    ...
    wandb.init(project="NNK prunning (CIFAR-10)", entity="carlowos", config = hparams)
    ...
    train_dataset = GNNBenchmarkDataset(root="./data/gcn/CIFAR10_norm", name="CIFAR10",
        split="train", pre_transform=T.Compose([T.NormalizeScale(), T.NormalizeFeatures()]))
    
    # Select 500 of class 0 and 500 from class 1
    idx0 = [i for i, label in enumerate(train_dataset.data.y) if label == 0]
    idx0 = random.sample(idx0, 500)
    idx1 = [i for i, label in enumerate(train_dataset.data.y) if label == 1]
    idx1 = random.sample(idx1, 500)
    train_dataset = train_dataset[idx0+idx1]

    validation_dataset = GNNBenchmarkDataset(root="./data/gcn/CIFAR10_norm", name="CIFAR10",
